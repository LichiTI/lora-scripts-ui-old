...
const ANIMA_LORA_SECTIONS = [
  ...
  sec('anima-params', 'model', 'Anima 专用参数', 'Anima 专用的时间步、权重与 VAE 设置（严格对齐后端 anima-lora.ts）。', [
    { key: 'qwen3_max_token_length', type: 'number', label: 'Qwen3 最大 token（qwen3_max_token_length）', desc: 'Qwen3 最大 token 长度', defaultValue: 512, min: 1 },
    { key: 't5_max_token_length', type: 'number', label: 'T5 最大 token（t5_max_token_length）', desc: 'T5 最大 token 长度', defaultValue: 512, min: 1 },
    { key: 'timestep_sampling', type: 'select', label: '时间步采样（timestep_sampling）', desc: '默认与官方 Anima 训练器一致的 shift', defaultValue: 'shift', options: ['sigma', 'uniform', 'sigmoid', 'shift', 'flux_shift'] },
    { key: 'sigmoid_scale', type: 'number', label: 'sigmoid 缩放（sigmoid_scale）', desc: 'sigmoid 缩放系数，默认 1.0', defaultValue: 1.0, step: 0.001 },
    { key: 'discrete_flow_shift', type: 'number', label: '离散流位移（discrete_flow_shift）', desc: 'Rectified Flow 位移，默认 3.0，与官方 Anima 训练器一致', defaultValue: 3.0, step: 0.001 },
    { key: 'min_timestep', type: 'number', label: '最小 timestep（min_timestep）', desc: '训练时允许的最小 timestep，留空表示不限制', defaultValue: '', min: 0 },
    { key: 'max_timestep', type: 'number', label: '最大 timestep（max_timestep）', desc: '训练时允许的最大 timestep，留空表示不限制', defaultValue: '', min: 1 },
    { key: 'weighting_scheme', type: 'select', label: '权重策略（weighting_scheme）', desc: '时间步分布权重策略', defaultValue: 'uniform', options: ['sigma_sqrt', 'logit_normal', 'mode', 'cosmap', 'none', 'uniform'] },
    { key: 'logit_mean', type: 'number', label: 'logit_normal 均值（logit_mean）', desc: 'weighting_scheme 为 logit_normal 时的均值，留空使用默认', defaultValue: '', step: 0.01 },
    { key: 'logit_std', type: 'number', label: 'logit_normal 标准差（logit_std）', desc: 'weighting_scheme 为 logit_normal 时的标准差，留空使用默认', defaultValue: '', step: 0.01 },
    { key: 'mode_scale', type: 'number', label: 'mode 权重缩放（mode_scale）', desc: 'mode 权重策略的缩放系数', defaultValue: '', step: 0.01 },
    { key: 'attn_mode', type: 'select', label: 'Attention 实现（attn_mode）', desc: '留空时按当前运行时自动选择；FlashAttention 运行时下 Anima 会优先尝试 FlashAttention 2', defaultValue: '', options: ['', 'torch', 'xformers', 'sageattn', 'flash'] },
    { key: 'split_attn', type: 'boolean', label: '拆分 attention（split_attn）', desc: '拆分 attention 计算以降低显存占用，但通常会牺牲训练速度', defaultValue: false },
    { key: 'vae_chunk_size', type: 'number', label: 'VAE 分块大小（vae_chunk_size）', desc: 'VAE 编码/解码分块大小，需为偶数', defaultValue: '', min: 2 },
    { key: 'vae_disable_cache', type: 'boolean', label: '禁用 VAE 缓存（vae_disable_cache）', desc: '禁用内部 VAE 缓存机制', defaultValue: false },
    { key: 'unsloth_offload_checkpointing', type: 'boolean', label: 'Unsloth Offload（unsloth_offload_checkpointing）', desc: '使用更快的 CPU RAM activation offload，不能与 blocks_to_swap / cpu_offload_checkpointing 同时使用', defaultValue: false },
  ]),
  sec('save-settings', 'model', '保存设置', '', [...S_SAVE]),
  sec('dataset-settings', 'dataset', '数据集设置', '', ds('1024,1024', 2048, 64)),
  sec('caption-settings', 'dataset', 'Caption 选项', '', S_CAPTION.filter((f) => f.key !== 'max_token_length')),
  sec('data-aug-settings', 'dataset', '数据增强', '颜色、翻转与裁剪增强。', [...S_DATA_AUG]),
  sec('network-settings', 'network', '网络设置', '适配器类型覆盖原生 LoRA 家族（LoRA/LoRA-FA/VeRA/T-LoRA）与 LyCORIS 路线（LoKr/LoHa/BOFT/GLoRA/GLoKr）。字段严格对齐后端 anima-lora.ts。', [
    { key: 'lora_type', type: 'select', label: '适配器类型（lora_type）', desc: 'LoRA 基础；LoRA-FA 冻结 lora_down；VeRA 共享随机投影；T-LoRA 动态 rank；LoKr 线性注入实验路线；LoHa/BOFT/GLoRA 走 LyCORIS 路线；GLoKr 为本仓库自定义混合路线，当前优先支持 Anima 主干 Linear 层', defaultValue: 'lora', options: ['lora', 'lora_fa', 'vera', 'tlora', 'lokr', 'loha', 'boft', 'glora', 'glokr'] },
    ...
    { key: 'dim_from_weights', type: 'boolean', label: '从权重推断 Dim（dim_from_weights）', desc: '从已有 network_weights 自动推断 rank / dim', defaultValue: false },
    { key: 'scale_weight_norms', type: 'number', label: '最大范数正则化（scale_weight_norms）', desc: '最大范数正则化。如果使用，推荐为 1', defaultValue: '', min: 0, step: 0.01 },
    { key: 'train_norm', type: 'boolean', label: '训练 Norm 层（train_norm）', desc: '额外训练带可学习参数的归一化层（如 RMSNorm/LayerNorm 的 weight/bias）。LyCORIS 路线（LoHa/BOFT/GLoRA/GLoKr）下受算法限制，IA3 类不计入', defaultValue: false },
    { key: 'network_dropout', type: 'number', label: 'Dropout（network_dropout）', desc: '原生 LoRA 家族的 dropout 概率', defaultValue: 0, min: 0, step: 0.01, visibleWhen: (c) => ['lora', 'lora_fa', 'vera', 'tlora'].includes(c.lora_type) },
    { key: 'dora_wd', type: 'boolean', label: '启用 DoRA（dora_wd）', desc: '原生 LoRA 或 LyCORIS（LoHa/BOFT/GLoRA）路线下启用 DoRA，把权重分解为方向与幅度两部分分别微调，更接近全量微调表现', defaultValue: false, visibleWhen: (c) => c.lora_type === 'lora' || ['loha', 'boft', 'glora'].includes(c.lora_type) },
    { key: 'wd_on_output', type: 'boolean', label: 'DoRA 输出侧范数（wd_on_output）', desc: 'LyCORIS DoRA 范数统计沿输出通道计算', defaultValue: true, visibleWhen: (c) => ['loha', 'boft', 'glora'].includes(c.lora_type) && c.dora_wd },
    { key: 'bypass_mode', type: 'boolean', label: 'Bypass Mode（bypass_mode）', desc: '兼容字段。启用 DoRA 时建议关闭；GLoKr 高级项里也可手动开启', defaultValue: false, visibleWhen: (c) => (c.lora_type === 'lora' && !c.dora_wd) || ['loha', 'boft', 'glora'].includes(c.lora_type) || (c.lora_type === 'glokr' && c.glokr_show_advanced) },
    ...
    { key: 'tlora_rank_schedule', type: 'select', label: 'T-LoRA Rank 调度（tlora_rank_schedule）', desc: 'T-LoRA 动态 rank 调度策略', defaultValue: 'cosine', options: ['cosine', 'linear'], visibleWhen: when('lora_type', 'tlora') },
    { key: 'tlora_orthogonal_init', type: 'boolean', label: 'T-LoRA 正交初始化（tlora_orthogonal_init）', desc: '对 lora_down 使用正交初始化（实验性）', defaultValue: false, visibleWhen: when('lora_type', 'tlora') },
    { key: 'lokr_factor', type: 'number', label: 'LoKr 系数（lokr_factor）', desc: '原生 LoKr 分解因子，常用 4~无穷（-1 为无穷）。GLoKr 高级项里作为 Kronecker 分解因子复用', defaultValue: 8, min: -1, visibleWhen: (c) => c.lora_type === 'lokr' || (c.lora_type === 'glokr' && c.glokr_show_advanced) },
    { key: 'dropout', type: 'number', label: 'LyCORIS Dropout（dropout）', desc: 'LyCORIS 主 dropout 概率', defaultValue: 0, min: 0, step: 0.01, visibleWhen: (c) => ['loha', 'boft', 'glora', 'glokr'].includes(c.lora_type) },
    { key: 'rank_dropout', type: 'number', label: 'Rank Dropout（rank_dropout）', desc: '按 rank 维度随机丢弃的概率', defaultValue: '', min: 0, max: 1, step: 0.01, visibleWhen: (c) => ['loha', 'boft', 'glora', 'glokr'].includes(c.lora_type)},
    { key: 'module_dropout', type: 'number', label: 'Module Dropout（module_dropout）', desc: '按整个模块随机丢弃的概率', defaultValue: '', min: 0, max: 1, step: 0.01, visibleWhen: (c) => ['loha', 'boft', 'glora', 'glokr'].includes(c.lora_type) },
    { key: 'use_scalar', type: 'boolean', label: 'Scalar 参数化（use_scalar）', desc: 'GLoRA / GLoKr 的 scalar 参数化。关闭时对应分支从 0 起步更稳；开启后从可学习缩放起步，前期更活跃', defaultValue: false, visibleWhen: (c) => ['glora', 'glokr'].includes(c.lora_type) },
    { key: 'rs_lora', type: 'boolean', label: 'rsLoRA 缩放（rs_lora）', desc: '把缩放从 alpha/rank 改成 alpha/sqrt(rank)，通常更适合较大 rank', defaultValue: false, visibleWhen: (c) => ['glora', 'glokr'].includes(c.lora_type) },
    { key: 'constraint', type: 'number', label: 'Constraint（constraint）', desc: 'BOFT 约束强度。用于限制旋转矩阵更新幅度；留空或 0 表示不额外约束', defaultValue: '', step: 0.01, visibleWhen: when('lora_type', 'boft') },
    { key: 'rescaled', type: 'boolean', label: 'Rescaled（rescaled）', desc: '启用 BOFT 的 rescaled 缩放参数，额外学习一组逐输出通道缩放', defaultValue: false, visibleWhen: when('lora_type', 'boft') },
    { key: 'full_matrix', type: 'boolean', label: 'Full Matrix（full_matrix）', desc: 'GLoKr 强制 Kronecker 分支 w1/w2 走 full-matrix 直存，不再用低秩分解。参数量和显存都会更高', defaultValue: false, visibleWhen: when('lora_type', 'glokr') },
    { key: 'glokr_show_advanced', type: 'boolean', label: '显示 GLoKr 高级项（glokr_show_advanced）', desc: '显示 Kronecker 分解细节与实验开关', defaultValue: false, visibleWhen: when('lora_type', 'glokr') },
    { key: 'decompose_both', type: 'boolean', label: '双侧分解（decompose_both）', desc: '允许 Kronecker 分支把较小的 w1 那一路也改成低秩分解。仅在 full_matrix 关闭且 rank 足够小时实际生效', defaultValue: false, visibleWhen: (c) => c.lora_type === 'glokr' && c.glokr_show_advanced },
    { key: 'unbalanced_factorization', type: 'boolean', label: '非均衡分解（unbalanced_factorization）', desc: 'Kronecker 分解时交换输出侧的分块方向，参数量通常不变，属于实验性结构开关', defaultValue: false, visibleWhen: (c) => c.lora_type === 'glokr' && c.glokr_show_advanced },
